...
async function main() {
  ...
  const report: Record<string, unknown> = {
    ...
    bakedRoute: manifest.foldRoute?.execution ?? '(not recorded)',
    bakedWorstTileBytes: manifest.foldRoute?.worstTileBytes ?? null,
    bakedBudgetBytes: manifest.foldRoute?.budgetBytes ?? null,
